...
@Service
public class CorporatepayrollservicesRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CorporatepayrollservicesRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.corporatepayrollservices.hosturl}")
	private String url;
	
	@Value("${spring.api.corporatepayrollservices.context-path}")
	private String contextPath;

	@Value("${spring.api.corporatepayrollservices.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CorporatepayrollservicesRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CorporatepayrollservicesRestProcessor.class.getName());
