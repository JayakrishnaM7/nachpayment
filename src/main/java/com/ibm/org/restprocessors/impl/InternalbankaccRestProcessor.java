...
@Service
public class InternalbankaccRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InternalbankaccRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.internalbankacc.hosturl}")
	private String url;
	
	@Value("${spring.api.internalbankacc.context-path}")
	private String contextPath;

	@Value("${spring.api.internalbankacc.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InternalbankaccRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InternalbankaccRestProcessor.class.getName());
