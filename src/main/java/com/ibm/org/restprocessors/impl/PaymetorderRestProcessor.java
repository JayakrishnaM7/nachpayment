...
@Service
public class PaymetorderRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PaymetorderRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.paymetorder.hosturl}")
	private String url;
	
	@Value("${spring.api.paymetorder.context-path}")
	private String contextPath;

	@Value("${spring.api.paymetorder.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PaymetorderRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PaymetorderRestProcessor.class.getName());
